...
const PLUGIN_NAME = 'JSON 工作台';

const services = {
  storage: {
    get: (key) => {
      try { return ipcRenderer.sendSync('plugin-storage-get', PLUGIN_NAME, key); }
      catch (_) { return null; }
    },
    set: (key, value) => {
      try { ipcRenderer.sendSync('plugin-storage-set', PLUGIN_NAME, key, value); }
      catch (_) {}
    }
  },
  window: {
    ...
    args: {
      enter: (action) => {
        const payload = action && typeof action === 'object'
          ...
          : '';
        if (payload) {
          window.dispatchEvent(new CustomEvent('plugin-enter', { detail: payload }));
        }
      }
    }
  }
