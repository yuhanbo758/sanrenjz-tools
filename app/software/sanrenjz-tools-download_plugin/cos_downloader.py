            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                for name in name_list:
                    try:
                        ...
                        # 解压单个文件
                        zip_ref.extract(name, self.software_dir)
                        
                        # 如果文件名需要重命名
                        if real_name != name:
                            old_path = os.path.join(self.software_dir, name)
                            new_path = os.path.join(self.software_dir, real_name)
                            # 确保目标目录存在
    ...
    def download_file(self, url):
        try:
            ...
            if file_name.lower().endswith('.zip'):
                ...
                if self.extract_zip(download_path):
                    self.status_label.config(text="文件解压完成！")
                else:
                    self.status_label.config(text="文件解压失败！")
            else:
