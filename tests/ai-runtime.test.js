...
async function main(){
  ...
  try{
    ...
    result=await api.complete({requestId:'provider-selection',selection:{providerId:'deepseek',modelId:'deepseek-chat'},stream:false,messages:[{role:'user',content:'MODEL'}]});assert.strictEqual(result.text,'model:deepseek-chat');
    ...
    console.log('AI runtime mock tests passed: stream, non-stream, provider selection, auth, timeout, cancel, capability guard');
